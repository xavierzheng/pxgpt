...
- **The shard *count* is not forced here, but `pxgpt shard-schema` still is.** The grammar-size limit that forces sharding is Anthropic's: `gpt-5.6-luna` accepts all 49 traits in a *single* shard (measured at 159 object properties, 165 enum values and depth 4, against OpenAI's limits of 5,000 / 1,000 / 10). What you cannot skip is the generator, because it is what *injects* `not_assessable` into every nominal and ordinal enum — in the 02_mature_v1 master, 45 of 45 such traits get it added and none list it themselves. Raising `--shard-budget` can cut OpenAI input cost while preserving the generated field and value definitions. Keep the chosen shard set fixed within a formal comparison, and do not feed a hand-written master schema directly to `--schema`. Sizing, cost and the accuracy evidence are worked through in [`dispatch_batch_vs_sequential.md`](dispatch_batch_vs_sequential.md) → *OpenAI → `--shard-budget` is the real cost lever*.
